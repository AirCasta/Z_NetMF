...
def predict_cv(X, y, train_ratio=0.2, n_splits=10, random_state=0, C=1.):
    ...
    logger.info("Average micro %.2f, Average macro %.2f",
            np.mean(micro) * 100,
            np.mean(macro) * 100)
    return [len(micro), train_ratio, np.mean(micro) * 100, np.mean(macro) * 100]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--stop-train-ratio", type=int, default=20,
            help="the end value of the train ratio (inclusive).")
    parser.add_argument("--num-train-ratio", type=int, default=2,
    ...
    df = pd.DataFrame(columns=['fold', 'ratio', 'micro', 'macro'])
    ...
    if not os.path.exists('.\\res\\'):
        os.makedirs('.\\res\\')
    df.to_csv('.\\res\\'+os.path.splitext(args.embedding.split('\\')[-1])[0]+'.csv')
